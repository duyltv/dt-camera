...
|---|---|---|
| `SESSION_TTL_HOURS` | `168` | Session lifetime in hours (one week). |
...
HLS is generated on demand by backend-owned `ffmpeg` processes. Output is stored
under the Docker-mounted `HLS_ROOT`, defaulting to `/tmp/dt-camera-hls`, and
served as `/hls/{camera_id}/index.m3u8`. Streams are reused while active and
stopped after `HLS_INACTIVITY_SECONDS`.
...
system event logging, a recorder status API, retention cleanup, and a basic
admin dashboard.
